...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, deconv):
        super(_DenseLayer, self).__init__()
        if deconv:
            self.add_module('relu1', nn.ReLU(inplace=True)),
            self.add_module('conv1', deconv(num_input_features, bn_size *
                                               growth_rate, kernel_size=1, stride=1)),
            self.add_module('relu2', nn.ReLU(inplace=True)),
            self.add_module('conv2', deconv(bn_size * growth_rate, growth_rate,
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate,deconv):
        super(_DenseBlock, self).__init__()
        for i in range(num_layers):
            layer = _DenseLayer(num_input_features + i * growth_rate, growth_rate,
                                bn_size, drop_rate,deconv)
            self.add_module('denselayer%d' % (i + 1), layer)


class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features,deconv):
        ...
        if not deconv:
            ...
            self.add_module('relu', nn.ReLU(inplace=True))
            self.add_module('conv', deconv(num_input_features, num_output_features,
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            block = _DenseBlock(num_layers=num_layers, num_input_features=num_features,
                                ...
                                drop_rate=drop_rate,deconv=deconv)
            self.features.add_module('denseblock%d' % (i + 1), block)
            num_features = num_features + num_layers * growth_rate
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=num_features,
                                    num_output_features=num_features // 2,deconv=deconv)
